        ...
        <div class="timetable-grid" :aria-label="'Timetable for ' + selectedDayLabel" role="grid">
          ...
          <div class="timetable-body" :style="{ height: timelineHeight + 'px' }">
            ...
            <button
              ...
              :style="gigFavStyle(renderedFavGig.gig)"
...
const favsRenderedGigsForDay = computed(() => {
  return favsGigsForDay.value.map((g) => ({
    gig: g,
    ...gigFavStyle(g) as Partial<RenderedGig>,
  }));
});
...
function getConcurrentColumns(gig: Gig): { concurrentCount: number; concurrentIndex: number } {
  ...
  const events = overlappingGigs.flatMap((candidate) => [
    { time: candidate.startTimestamp, delta: 1 },
    { time: candidate.endTimestamp, delta: -1 },
  ]);
